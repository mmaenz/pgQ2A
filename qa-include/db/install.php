...
$install_tables = array(
    'users' => array('CREATE TABLE users (
                        userid SERIAL CHECK (userid > 0) NOT NULL, 
                        created TIMESTAMP(0) NOT NULL, 
			createip INT CHECK (createip > 0) NOT NULL, 
                        email VARCHAR(80) NOT NULL, 
                        handle VARCHAR(20) NOT NULL, 
                        avatarblobid BIGINT CHECK (avatarblobid > 0), 
                        avatarwidth SMALLINT CHECK (avatarwidth > 0), 
			avatarheight SMALLINT CHECK (avatarheight > 0), 
			passsalt BYTEA, passcheck BYTEA, 
			level SMALLINT CHECK (level > 0) NOT NULL, 
			loggedin TIMESTAMP(0) NOT NULL, 
			loginip INT CHECK (loginip > 0) NOT NULL, 
			written TIMESTAMP(0), 
			writeip INT CHECK (writeip > 0), 
			emailcode CHAR(8) NOT NULL DEFAULT ' . qa_db_escape_string('') . ', 
			sessioncode CHAR(8) NOT NULL DEFAULT ' . qa_db_escape_string('') . ', 
			sessionsource VARCHAR (16) DEFAULT ' . qa_db_escape_string('') . ', 
			flags SMALLINT CHECK (flags > 0) NOT NULL DEFAULT 0, 
			wallposts INT NOT NULL DEFAULT 0, 
			PRIMARY KEY (userid));',
		    'CREATE INDEX ON users (email);',
		    'CREATE INDEX ON users (handle);',
		    'CREATE INDEX ON users (level);',
		    'CREATE INDEX ON users (created, level, flags);'),
    'userlogins' => array('CREATE TABLE userlogins (
                            userid INT CHECK (userid > 0) NOT NULL, 
			    source VARCHAR (16) NOT NULL, 
			    identifier BYTEA NOT NULL, 
			    identifiermd5 BYTEA NOT NULL, 
			    CONSTRAINT userlogins_ibfk_1 FOREIGN KEY (userid) REFERENCES users(userid) ON DELETE CASCADE);',
			    'CREATE INDEX ON userlogins (source, identifiermd5);',
			    'CREATE INDEX ON userlogins (userid);'),
    'userlevels' => array('CREATE TABLE userlevels (
                            userid INT CHECK (userid > 0) NOT NULL UNIQUE, 
                            entitytype CHAR(1) NOT NULL UNIQUE, 
                            entityid INT CHECK (entityid > 0) NOT NULL UNIQUE, 
                            "level" SMALLINT CHECK ("level" > 0), 
                            CONSTRAINT userlevels_ibfk_1 FOREIGN KEY (userid) REFERENCES users(userid) ON DELETE CASCADE);',
			    'CREATE INDEX ON userlevels (entitytype, entityid);'),
    'userprofile' => array('CREATE TABLE userprofile (
			    userid INT NOT NULL UNIQUE, 
			    title VARCHAR(40) NOT NULL, 
			    "content" VARCHAR(8000) NOT NULL, 
			    CONSTRAINT userprofile_ibfk_1 FOREIGN KEY (userid) REFERENCES users(userid) ON DELETE CASCADE);'),
    'userfields' => array('CREATE TABLE userfields (
                            fieldid SERIAL CHECK (fieldid > 0) NOT NULL, 
                            title VARCHAR(40) NOT NULL, 
                            "content" VARCHAR(40), position SMALLINT CHECK (position > 0) NOT NULL, 
                            flags SMALLINT CHECK (flags > 0) NOT NULL, 
                            permit SMALLINT CHECK (permit > 0), 
                            PRIMARY KEY (fieldid));'),
    'messages' => array('DROP TYPE IF EXISTS status;',
                        'CREATE TYPE status AS ENUM (' . qa_db_escape_string('PRIVATE') . ', ' . qa_db_escape_string('PUBLIC') . ');',
                        'CREATE TABLE messages (
                            messageid SERIAL CHECK (messageid > 0) NOT NULL, 
                            "type" status NOT NULL DEFAULT ' . qa_db_escape_string('PRIVATE') . ', 
                            fromuserid INT CHECK (fromuserid > 0) NOT NULL, 
                            touserid INT CHECK (touserid > 0) NOT NULL, 
                            fromhidden SMALLINT CHECK (fromhidden > 0) NOT NULL DEFAULT 0, 
                            tohidden SMALLINT CHECK (tohidden > 0) NOT NULL DEFAULT 0, 
                            "content" VARCHAR(8000) NOT NULL, 
                            format VARCHAR(20) NOT NULL, 
                            created TIMESTAMP(0) NOT NULL, 
                            PRIMARY KEY (messageid));',
			    'CREATE INDEX ON messages ("type", fromuserid, touserid, created);',
			    'CREATE INDEX ON messages (touserid, "type", created);',
			    'CREATE INDEX ON messages (fromhidden);',
			    'CREATE INDEX ON messages (tohidden);'),
    'userfavorites' => array('CREATE TABLE userfavorites (
                                userid INT CHECK (userid > 0) NOT NULL, 
                                entitytype CHAR(1) NOT NULL, 
                                entityid INT CHECK (entityid > 0) NOT NULL, 
                                nouserevents SMALLINT CHECK (nouserevents > 0) NOT NULL, 
                                PRIMARY KEY (userid, entitytype, entityid), 
                                CONSTRAINT userfavorites_ibfk_1 FOREIGN KEY (userid) REFERENCES users(userid) ON DELETE CASCADE);',
			    'CREATE INDEX ON userfavorites (userid, nouserevents);',
			    'CREATE INDEX ON userfavorites (entitytype, entityid, nouserevents);'),
    'usernotices' => array('CREATE TABLE usernotices (
                            noticeid SERIAL CHECK (noticeid > 0) NOT NULL, 
                            userid INT CHECK (userid > 0) NOT NULL, 
                            content VARCHAR(8000) NOT NULL, 
                            format VARCHAR(20) NOT NULL, 
                            tags VARCHAR(200), 
		           ...
		           'CREATE INDEX ON usernotices (userid, created);'),
    'userevents' => array('CREATE TABLE userevents (
                            userid INT CHECK (userid > 0) NOT NULL, 
                            entitytype CHAR(1) NOT NULL, 
                            entityid INT CHECK (entityid > 0) NOT NULL, 
                            questionid INT CHECK (questionid > 0) NOT NULL, 
                            lastpostid INT CHECK (lastpostid > 0) NOT NULL, 
                            updatetype CHAR(1), 
                            lastuserid INT CHECK (lastuserid > 0), 
                            updated TIMESTAMP(0) NOT NULL, 
                            CONSTRAINT userevents_ibfk_1 FOREIGN KEY (userid) REFERENCES users(userid) ON DELETE CASCADE);',
			    'CREATE INDEX ON userevents (userid, updated);',
			    'CREATE INDEX ON userevents (questionid, userid);'),
    'sharedevents' => array('CREATE TABLE sharedevents (
                                entitytype CHAR(1) NOT NULL, 
                                entityid INT CHECK (entityid > 0) NOT NULL, 
                                questionid INT CHECK (questionid > 0) NOT NULL, 
                                lastpostid INT CHECK (lastpostid > 0) NOT NULL, 
                                ...
                                lastuserid INT CHECK (lastuserid > 0), 
                                updated TIMESTAMP(0) NOT NULL);',
			    'CREATE INDEX ON sharedevents (entitytype, entityid, updated);',
			    'CREATE INDEX ON sharedevents (questionid, entitytype, entityid);'),
    'cookies' => array('CREATE TABLE cookies (
                        cookieid BIGINT CHECK (cookieid > 0) NOT NULL, 
                        created TIMESTAMP(0) NOT NULL, 
                        createip INT CHECK (createip > 0) NOT NULL, 
                        written TIMESTAMP(0), 
                        writeip INT CHECK (writeip > 0), 
                        PRIMARY KEY (cookieid));'),
    'categories' => array('CREATE TABLE categories (
			    categoryid SERIAL NOT NULL, 
			    parentid INT UNIQUE, 
			    title VARCHAR(80) NOT NULL, 
			    tags VARCHAR(200) NOT NULL, 
			    "content" VARCHAR(800) NOT NULL DEFAULT ' . qa_db_escape_string('') . ', 
			    qcount INT NOT NULL DEFAULT 0, 
			    "position" SMALLINT NOT NULL, 
			    backpath VARCHAR(804) NOT NULL DEFAULT ' . qa_db_escape_string('') . ', 
			    PRIMARY KEY (categoryid));',
			    'CREATE INDEX ON categories (backpath);'),
    'pages' => array('CREATE TABLE pages (
			...
			'CREATE INDEX ON pages (pageid);'),
    'widgets' => array('CREATE TABLE widgets (
...
function qa_install_table($table) {
 ...
 */ {
    $coltype = strtoupper(qa_get_PDO_user_column_type());
 ...
 */ {
    ...
    if (count($missing) == count($definitions))
        ...
        if (count($missing)) {
            if (defined('QA_PDO_USERS_PREFIX')) { // special case if two installations sharing users
                $datacount = 0;
                $datamissing = 0;

                foreach ($definitions as $rawname => $definition)
                    if (qa_db_add_table_prefix($rawname) == (QA_PDO_TABLE_PREFIX . $rawname)) {
                        $datacount++;
 ...
 */ {
    ...
    global $install_tables;

    foreach ($missingtables as $rawname => $definition) {
        $table_array = $install_tables[$rawname];

        if ($table_array !== null) {
            qa_install_table($table_array);
        } else {
